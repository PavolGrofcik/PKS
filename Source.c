			...
			while (pcap_next_ex(f, &header, &pktdata) > 0) {
				if (tmp == arr[pom] && count <20) {
					if (count % 2 == 0) {
						...
						Print_info(header, pktdata, first, arr, pom);		//ZobrazÌ inform·cie o danej komunik·cii
...
// Ak otvaraö in˝ s˙bor musÌö zmenit v main,switch-rewind point 1 a vypis-IP,arp komunik·ciu

int main(void) {

	char errbuff[PCAP_ERRBUF_SIZE];
	char path[] = "C:\\Users\\Pavol GrofËÌk\\Documents\\Visual Studio 2017\\Projects\\Winpcap\\Winpcap\\trace-17.pcap";
