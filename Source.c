...
#define FILENAME	"eth-4.pcap"			//N·zov analyzovanÈho pcap s˙boru
...
#define NAME_LEN	16						//Max dÂûka pre n·zov protokolov
...
int init(DataLink **link, Arp **arp, Ip **ip, Tcp **tcp) {
	...
	*tcp = (Tcp*)malloc(sizeof(Tcp));
	...
	read_protocols(&fr, *link, *arp, *ip, *tcp);					//NaËÌtanie protokolov zo s˙boru PROTOCOLS
...
//Funkcia k v˝pisu tcp komunik·ciÌ
